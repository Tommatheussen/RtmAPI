...
if __name__ == '__main__':
    ...
    result = api.rtm.tasks.getList(last_sync="2013")
    for tasklist in result.tasks:
        for taskseries in tasklist:
            print taskseries.task.id, taskseries.task.due, taskseries.name
        for taskseries in tasklist.deleted:
            print "DELETED:", taskseries.task.id
